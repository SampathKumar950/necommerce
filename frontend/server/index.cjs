  ...
  new GoogleStrategy(
    ...
    async (accessToken, refreshToken, profile, done) => {
      try {
        ...
        const query = "INSERT INTO users (name, email, password) VALUES ($1, $2, $3) ON CONFLICT (email) DO NOTHING RETURNING *;"
...
passport.deserializeUser(async (id, done) => {
  try {
    ...
    done(null, user.rows[0]); // Retrieve user from database
  } catch (err) {
...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: newUser.rows[0].id, email },
...
app.post("/signin", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: user.rows[0].id, email },
         process.env.JWT_SECRET || "bfa377cc789876877ac51aad201bcf9a90eba0a2d1654d4e8076a1c956d40b86",
...
// Logout Route inka button pettaledu kabatti routes tho logout avutam insert button and avoid user accessing url directly
